...
const MeanSpeedCard = ({ target, className }: MeanSpeedCardProps) => {
  ...
  return (
    <div className={cn("glass-card relative rounded-2xl p-6 flex flex-col", className)}>
      <Target className={cn("w-8 h-8 mb-1", target.calibrating ? "text-muted-foreground" : accent)} strokeWidth={1.5} />
      <p className="text-muted-foreground text-sm font-medium mb-1 uppercase tracking-wide">
        Target Avg
      </p>
      ...
      <div className="flex-1 flex flex-col justify-end">
        {target.finished ? (
          ...
          <>
            <span className="text-4xl font-bold font-mono text-muted-foreground leading-none">--</span>
            <span className="text-xs text-muted-foreground">calibrating…</span>
          </>
        ) : (
          <>
            <div className="flex items-baseline gap-1">
              <span className={cn("text-4xl font-bold font-mono leading-none", accent)}>
                {Math.round(target.targetKmh)}
              </span>
              <span className="text-sm text-muted-foreground">km/h</span>
            </div>
            <span className={cn("text-xs font-mono font-bold", accent)}>
